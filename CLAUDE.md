...
1. **DO memory** (hot) — `pendingChunks[]` accumulates binary frames during live broadcast.
2. **R2 (incremental)** — `flushPendingToR2()` writes numbered part files (`part-000000.json`, etc.) every 30s via alarm.
