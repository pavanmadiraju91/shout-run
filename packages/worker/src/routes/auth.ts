...
// GET /api/auth/github/callback - Exchange code for token, redirect to web app
auth.get('/github/callback', async (c) => {
  const code = c.req.query('code');
  const error = c.req.query('error');

  // Determine the web app origin to redirect back to
  const host = c.req.header('Host') || '';
  const webOrigin = host.includes('localhost')
    ? 'http://localhost:3000'
    : 'https://shout.run';

  if (error || !code) {
    return c.redirect(`${webOrigin}/auth/callback?error=${encodeURIComponent(error || 'no_code')}`);
  }
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      return c.redirect(`${webOrigin}/auth/callback?error=token_exchange_failed`);
    }
    ...
    if (tokenData.error || !tokenData.access_token) {
      return c.redirect(`${webOrigin}/auth/callback?error=${encodeURIComponent(tokenData.error || 'no_token')}`);
    }
    ...
    if (!userResponse.ok) {
      return c.redirect(`${webOrigin}/auth/callback?error=github_user_fetch_failed`);
    }
    ...
    // Redirect to web app with token in URL fragment (not query param, so it stays client-side)
    const params = new URLSearchParams({
      token: jwt,
      username: user.username,
      avatarUrl: user.avatarUrl,
    ...
    return c.redirect(`${webOrigin}/auth/callback?${params.toString()}`);
  } catch (err) {
    console.error('OAuth callback error:', err);
    return c.redirect(`${webOrigin}/auth/callback?error=internal_error`);
  }
});
